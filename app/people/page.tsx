...
export default function People() {
    ...
    <>
        <div className="flex flex-col w-full">
            ...
            {members.map((member, index) => (
                <>
                    ...
                    <div className="flex flex-col md:flex-row w-11/12 md:w-3/4 mx-auto">
                        ...
                        <img src={`/img/member/${member.image}`} alt="Mike Morgan" className="rounded-lg w-56 h-56" />
                        </div>
                        ...
                        {member.email && (
                            <Link href={`mailto:${member.email}`}>
                                <img src="/img/social/email.png" alt="Email" className="w-6 h-6 mr-2" />
                            </Link>
                        )}
                        {member.twitter && (
                        ...
                        {member.website && (
                            <Link href={member.website}>
                                <img src="/img/social/website.png" alt="Website" className="w-6 h-6 mr-2" />
                            </Link>
                        )}
                        {member.github && (
            ...
            <div className="flex flex-col w-full">
            <div className="flex flex-col w-11/12 md:w-3/4 mx-auto">
                <h1 className="text-center mt-12 text-2xl md:text-3xl lg:text-4xl font-bold mb-4">Alumni</h1>
            </div>
            </div>
            <hr className="border-t border-gray-600 w-11/12 md:w-3/4 mx-auto mb-8" />
            <div className="flex flex-col w-full">
            <div className="flex flex-col w-11/12 md:w-3/4 mx-auto">
                <ul className="list-disc">
                    {alumni.map((alumnus, index) => (
                        <li key={index}>{alumnus.name} ({getYear(alumnus.join_date)})- ({alumnus.project})</li>
                    ))}
                </ul>
            </div>
            </div>
